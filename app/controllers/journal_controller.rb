...
before_action :authenticate_user!
  ...
  def public
    @public = Journal.where(public: true)
    render layout: 'homepage'
  end
  ...
  def create
    @journal = Journal.new(params.require(:journal).permit(:name, :user_id, :image, :public))
  ...
  def update
    ...
    if @journal.update_attributes(params.require(:journal).permit(:name, :user_id, :image, :public))
      redirect_to journal_path
    else
      render :edit
    end
